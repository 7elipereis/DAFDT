function [] = plotOverlapped(DS,Lims,Weights,Labels,filename,plottitle)
    ...
    %prints just disagreement
    
    for idta=1:size(DTA,1)
        if and((DTA(idta,3) == 0),(DTA(idta,4) < 1))
            %plot red
            scatter(DTA(idta,1),DTA(idta,2), 10,  'filled','o', 'MarkerFaceColor', 'red', 'MarkerFaceAlpha', DTA(idta,4));
            hold on;
        end
        if and((DTA(idta,3) == 1),(DTA(idta,4) < 1))
            %plot blue
            scatter(DTA(idta,1),DTA(idta,2), 10,  'filled','o', 'MarkerFaceColor', 'blue', 'MarkerFaceAlpha', DTA(idta,4));
            hold on;
        end        
    ...
    xlabel('A');
    ylabel('B');
    title(strcat(plottitle, ' Disagrements'));
    xlim([0 100]);
    ylim([0 100])
    xticks(0:10:100)
    yticks(0:10:100)
    ...
    %works to print on file from java
    print(strcat(filename, 'Disagrements'), '-dpng');
    %doenst work to print on file from java
    %saveas(fig, "plot.png", "png");
    clf;
